...
def DeepW(file_name, kmer, seq_kernel, struc_kernel):
    ...
    # 随机下采样0
    seq_resampled, struc_resampled, label_resampled = undersampling.custom_random_undersampling(file_name,
                                                                                                train_data[0][0],
                                                                                                train_data[0][1],
                                                                                                train_label,
                                                                                                target_ratio=1.2,
    ...
    if kmer == 1:
        ...
        dw_model = multi_model.only_BLSTM_CNN_2mer(train_seq.shape[1], train_struc.shape[1], seq_kernel, struc_kernel)
    ...
    dw_model.fit(x=[train_seq, train_struc], y=train_y, batch_size=50, epochs=30,
                 validation_data=([val_seq, val_struc], val_y), verbose=1, shuffle=True)
    end_time = time.time()
    train_time = end_time - start_time
    print("训练时间：", train_time, 's')

    prob_val = dw_model.predict([val_seq, val_struc])
    ...
    # save 2mer model
    # 1dw_model.save(model_path+file_name[:10]+'.h5')
    ...
    p, sn, sp, acc, balanced_acc, AUC, mcc = score.calculate_performace(len(y_pred_cnn), y_pred_cnn, y_prob,
                                                                        test_y[:, -1])
    print(str(kmer) + 'mer_model:\t' + 'P:' + str(p) + '\t' + 'SN:' + str(sn) + '\t' + 'SP:' + str(
        sp) + '\t' + 'ACC:' + str(acc) + '\t' + 'balanced_acc:' + str(balanced_acc) + '\t' + 'AUC:' + str(
        AUC) + '\t' + 'MCC:' + str(mcc) + '\n')
    with open('./results/' + str(kmer) + 'mer_model_kernel_' + str(seq_kernel) + '_' + str(
            struc_kernel) + '_BLSTM_compared.txt', 'a') as fw:
        fw.write(file_name + '_' + str(seq_kernel) + '_' + str(struc_kernel) + '\tP:' + str(p) + '\t' + 'SN:' + str(
            sn) + '\t' + 'SP:' + str(sp) + '\t' + 'ACC:' + str(acc) + '\t' + 'balanced_acc:' + str(
            balanced_acc) + '\t' + 'AUC:' + str(AUC) + '\t' + 'MCC:' + str(mcc) + '\n')
...
if __name__ == '__main__':
    ...
    seq_kernel = 4
    struc_kernel = 4
    dataset = '15_PARCLIP_IGF2BP123_hg192'
